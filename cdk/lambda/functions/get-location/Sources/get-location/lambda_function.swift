...
@main
struct GetLocationFunction: SimpleLambdaHandler {
    ...
    func handle(_ event: Event, context: LambdaContext) async throws -> Location {
        ...
        return getLocation(item: response.results![0])
    }
}
...
func getLocation (item: LocationClientTypes.SearchForPositionResult) -> Location {
